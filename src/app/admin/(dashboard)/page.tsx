...
import { ViewsChart } from "@/components/admin/AdminCharts";
  ...
  return (
    <div className="space-y-10">
      <div className="slide-up">
        ...
        <h1 className="font-heading text-2xl font-bold text-text text-balance">Panel de control</h1>
            ...
            {recentReviews?.length ? (
              <div className="space-y-0 divide-y divide-border/50">
                {recentReviews.map((r, i) => (
                  <div
                    key={r.id}
                    className="flex items-center gap-4 py-3.5 first:pt-0 last:pb-0 group stagger-fade"
              ...
              <div className="text-center py-12">
                <div className="w-12 h-12 rounded-xl bg-surface-elevated flex items-center justify-center mx-auto mb-4">
                  <Star className="h-6 w-6 text-text-muted/30" strokeWidth={1.5} />
                </div>
      ...
      <div className="admin-card">
        <div className="p-6">
          ...
          <div className="mb-6">
            <ViewsChart data={(tours ?? []).slice(0, 6).map((t) => ({ name: t.title_es?.split(" ")[0] ?? "—", views: t.views ?? 0 }))} />
          </div>
...
function MetricBox({ value, label, sub, accent }: { value: number; label: string; sub: string; accent: string }) {
  const dotColor = accentText[accent] ?? "text-emerald";
  return (
    <div className={`rounded-xl ${accentBgs[accent] ?? "bg-emerald-dim"} p-5`}>
      ...
      <p className={`mono-ui text-[9px] mt-1 flex items-center gap-1.5 ${dotColor}`}>
