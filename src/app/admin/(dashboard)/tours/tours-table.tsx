...
const CATEGORY_BGS: Record<string, string> = {
  day_park: "bg-emerald-dim border-emerald/20",
  mangrove: "bg-teal-500/10 border-teal-500/20",
  night_walk: "bg-amber-500/10 border-amber-500/20",
};

const CATEGORY_TEXT: Record<string, string> = {
  day_park: "text-emerald",
  mangrove: "text-teal-500",
  night_walk: "text-amber-500",
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((tour, i) => (
          <div
            key={tour.id}
            className="admin-card stagger-fade"
            style={{ animationDelay: `${i * 60}ms` }}
          >
            <div className={`p-5 border-t-[3px] ${CATEGORY_BGS[tour.category]?.split(" ")[1] ?? "border-emerald/20"}`}>
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <span className={`inline-flex items-center gap-1.5 mono-ui text-[9px] px-2 py-1 rounded-full shrink-0 ${CATEGORY_TEXT[tour.category] ? `${CATEGORY_BGS[tour.category]} border` : "bg-emerald-dim text-emerald border border-emerald/20"}`}>
        ...
        <div className="text-center py-16">
          {search || category !== "all" ? (
            <div>
              ...
              <p className="text-sm text-text-muted font-medium">No se encontraron tours con esos filtros</p>
              <p className="text-xs text-text-muted/50 mt-1">Probá con otros términos de búsqueda</p>
            </div>
          ) : (
            <div>
              <div className="w-14 h-14 rounded-2xl bg-surface-elevated flex items-center justify-center mx-auto mb-4">
                <Trees className="h-6 w-6 text-text-muted/30" strokeWidth={1.5} />
              </div>
              <p className="text-sm text-text-muted font-medium">No hay tours todavía</p>
              <p className="text-xs text-text-muted/50 mt-1">Creá el primer tour para empezar</p>
              <Link href="/admin/tours/new" className="inline-flex items-center gap-1.5 text-sm text-emerald hover:text-emerald-bright transition-colors mt-4 font-medium">
